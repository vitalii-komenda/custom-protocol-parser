...
export default process = (records) => {
    ...
    records.forEach(record => {
        ...
        if (record.type === RECORD_TYPE.Debit) {
            ...
            balanceByUser[record.userId] = balanceByUser[record.userId].add(Big(record.amountUSD));
        }
        if (record.type === RECORD_TYPE.Credit) {
            totalCredits = totalCredits.add(Big(record.amountUSD));

            balanceByUser[record.userId] = balanceByUser[record.userId].sub(Big(record.amountUSD));
        }
